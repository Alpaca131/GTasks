...
import { getScrollListDataConfig } from '../utils/getScrollListDataConfig'
...
Page({
  ...
  createAndUpdateList(showEmpty = true) {
    const _scrollListItemClick = (list, index) => {
      hmApp.gotoPage({ url: 'pages/taskDetails', param: JSON.stringify(dataList[index]) })
    }
    ...
    const titleList = dataList.map(({ title }) => title);
    if (scrollList) {
      scrollList.setProperty(hmUI.prop.UPDATE_DATA, {
        data_array: titleList,
